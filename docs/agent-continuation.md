...
| Dependency-advisory scan (`osv-scanner 2.5.1`, installed via scoop) | DONE — 0 vulnerabilities | `osv-scanner scan source --lockfile=pubspec.lock .` scanned 167 pub packages against the OSV database; JSON report `results: []`. Does not cover the already-tracked `docs/release.md` Kotlin/Gradle-plugin risk (`GHSA-r937-wjx7-w2jp` / CVE-2026-53914) since that's a build-toolchain dependency, not a `pub` package — that risk remains open per its existing entry. |
| SBOM (`docs/sbom.cdx.json`) | DONE | `osv-scanner`'s own `cyclonedx-1-6`/`spdx-2-3` exporters produced empty/placeholder output for this pub lockfile (a real limitation of osv-scanner 2.5.1 for the Dart ecosystem, not a config error — verified both formats). Generated a real CycloneDX 1.6 SBOM instead from `flutter pub deps --json` (the authoritative resolved dependency graph): 167 components, each as a `pkg:pub/<name>@<version>` purl tagged with its `pub:source` (hosted/sdk/git) and `pub:kind` (direct/dev/transitive). Committed at `docs/sbom.cdx.json`; regenerate after any `pubspec.lock` change. |
| Hosted CI run (`.github/workflows/quality.yml`) | DONE — both jobs green | Pushed `573ffa6` and `bf2f2af` to `origin/main`. Run `33960091383` (commit `d4eceec`) failed: Windows job's "Verify SDK versions" step crashed — `ConvertFrom-Json` choked on stdout because a freshly extracted SDK's first invocation prints one-time "Building flutter tool..."/"Running pub upgrade..." text ahead of the `--machine` JSON. Fixed by warming up with a throwaway `flutter --version` call first (`bf2f2af`). Re-ran as run [`33962009736`](https://github.com/nguyendong47/flutter-clean-notes/actions/runs/33962009736): `Windows desktop and SQLite FFI` (5m4s) and `Analyze, test, web, Android debug` (10m46s) both passed. |
| App Store / Play Store submission | PENDING | Requires external credentials and store accounts — unavailable in this session; owner action. |
